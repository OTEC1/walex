...
public class Silk {
	...
	public List<StreamingCont>  listSilk() {
		...
		try {
			...
			PreparedStatement pst=con.prepareStatement("select * from walex_uploads where cats='Silk'");
	...
	public  List <StreamingCont>list() {
		...
		try {
			 con=db.getConnection();
//			 
			PreparedStatement pst=con.prepareStatement("select * from walex_uploads where cats like '%Silk%'  order by id desc limit 6");
			ResultSet rs=pst.executeQuery();
