...
public class Brocade {
	...
	public List<StreamingCont>  listBrocade() {
		...
		try {
			...
			PreparedStatement pst=con.prepareStatement("select * from walex_uploads where cats='Brocade'");
	...
	public  List <StreamingCont>list() {
		...
		try {
			...
			PreparedStatement pst=con.prepareStatement("select * from walex_uploads where cats like '%Brocade%' order by id desc limit 5");
			ResultSet rs=pst.executeQuery();
